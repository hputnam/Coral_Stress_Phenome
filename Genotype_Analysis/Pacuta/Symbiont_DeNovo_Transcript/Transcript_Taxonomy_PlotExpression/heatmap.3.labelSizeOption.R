...
## Update by TGS on 26/7/2021
##	- Added option to function to adjust color sample panel label size
    ...
    if (!missing(RowSideColors)) {
        if (!is.matrix(RowSideColors)){
            ...
            if (length(colnames(RowSideColors)) > 0) {  
            	...
            	axis(1, 1:ncol(RowSideColors), labels=colnames(RowSideColors), las=2, cex.axis=colorbar.cex.axis, tick=F, xlab="", ylab="") ## TGS: Added adjustable cex.axis variable
    ...
    if (!missing(ColSideColors)) {
        ...
        if (!is.matrix(ColSideColors)){
            ...
            if (length(rownames(ColSideColors)) > 0) {
				...
				axis(2, 1:(nrow(ColSideColors)), labels=rownames(ColSideColors), las = 2, tick = FALSE, cex.axis=colorbar.cex.axis) ## TGS: Added adjustable cex.axis variable
            }
        }
    }
