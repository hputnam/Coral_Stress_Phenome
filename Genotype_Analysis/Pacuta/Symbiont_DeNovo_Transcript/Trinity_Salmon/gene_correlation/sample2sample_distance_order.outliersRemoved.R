...
matrix.file <- "Trinity.fasta.salmon.allSamples.numreads.matrix"
sample.file <- "samples_Pacuta.SNPclades.outliersRemoved.txt"
output.prefix <- "Trinity.fasta.salmon.allSamples.numreads.matrix.hclust_sample_order.outliersRemoved"
...
write(rev(hc_samples$labels[hc_samples$order]), file=paste(output.prefix,".txt",sep=''))
##
##
##

pdf(paste(output.prefix,".cor_heatmap.pdf",sep=''))
...
", paste(matrix.file,".minRow10.CPM.log2",sep='')) , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
