    ...
    if (!missing(RowSideColors)) {
        if (!is.matrix(RowSideColors)){
            ...
            if (length(colnames(RowSideColors)) > 0) {  
            	...
            	axis(1, 1:ncol(RowSideColors), labels=colnames(RowSideColors), las=2, cex.axis=0.1, tick=F, xlab="", ylab="")
    ...
    if (!missing(ColSideColors)) {
        ...
        if (!is.matrix(ColSideColors)){
            ...
            if (length(rownames(ColSideColors)) > 0) {
				...
				axis(2, 1:(nrow(ColSideColors)), labels=rownames(ColSideColors), las = 2, tick = FALSE, cex.axis=0.1)
            }
        }
    }
