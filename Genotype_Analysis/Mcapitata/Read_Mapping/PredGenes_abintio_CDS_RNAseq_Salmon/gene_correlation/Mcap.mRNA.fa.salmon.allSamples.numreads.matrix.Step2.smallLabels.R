...
if (file.exists("Mcap.mRNA.fa.salmon.allSamples.numreads.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("Mcap.mRNA.fa.salmon.allSamples.numreads.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("Mcap.mRNA.fa.salmon.allSamples.numreads.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
...
samples_data = read.table("samples_Mcapitata.txt", header=F, check.names=F, fill=T)
...
write.table(data, file="Mcap.mRNA.fa.salmon.allSamples.numreads.matrix.minRow10.CPM.log2.dat", quote=F, sep='	');
if (nrow(data) < 2) { stop("
...
write.table(sample_cor, file="Mcap.mRNA.fa.salmon.allSamples.numreads.matrix.minRow10.CPM.log2.sample_cor.dat", quote=F, sep='	')
sample_dist = dist(t(data), method='euclidean')
hc_samples = hclust(sample_dist, method='complete')
pdf("Mcap.mRNA.fa.salmon.allSamples.numreads.matrix.minRow10.CPM.log2.sample_cor_matrix.smallLabels.pdf")
sample_cor_for_plot = sample_cor
if (is.null(hc_samples)) { RowV=NULL; ColV=NULL} else { RowV=as.dendrogram(hc_samples); ColV=RowV }
heatmap.3(sample_cor_for_plot, dendrogram='both', Rowv=RowV, Colv=ColV, col = myheatcol, scale='none', symm=TRUE, key=TRUE,density.info='none', trace='none', symkey=FALSE, symbreaks=F, margins=c(10,10), cexCol=0.1, cexRow=0.1, cex.main=0.75, main=paste("sample correlation matrix
", "Mcap.mRNA.fa.salmon.allSamples.numreads.matrix.minRow10.CPM.log2") , ColSideColors=sampleAnnotations, RowSideColors=t(sampleAnnotations))
dev.off()
gene_cor = NULL
