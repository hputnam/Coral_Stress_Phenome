...
# 5210836 (46.01% remain after first-pass filtering)
zcat "${OUT}_INDELs.vcf.gz" | grep -v '^#' | wc -l
# 1897475
zcat "${OUT}_INDELs_VarScores_filter.vcf.gz" | grep 'PASS' | wc -l
# 668707 (35.24% remain after first-pass filtering)
...
while read i;
do
        cut -f $i,$((i+1)) "${OUT}_SNPs_VarScores_filterPASSED_DPfilterNoCall.DP.table" | awk '$1 != "./." && $1 != ".|." && $1 != "././." && $1 != ".|.|." {print $2}' > "${OUT}_SNPs_VarScores_filterPASSED_DPfilterNoCall.GT.DP.txt"
done < <(head -n 1 "${OUT}_SNPs_VarScores_filterPASSED_DPfilterNoCall.DP.table" | awk '{for (i=1; i<=NF; ++i) {if($i~".GT$") {print i}}}')
while read i;
do
        cut -f $i,$((i+1)) "${OUT}_INDELs_VarScores_filterPASSED_DPfilterNoCall.DP.table" | awk '$1 != "./." && $1 != ".|." && $1 != "././." && $1 != ".|.|." {print $2}' > "${OUT}_INDELs_VarScores_filterPASSED_DPfilterNoCall.GT.DP.txt"
...
do
        ...
        cut -f $i-$((i+2)) "${OUT}_SNPs_VarScores_filterPASSED_DPfilterNoCall.AD.table" | awk '$1 != "./." && $1 != ".|." && $1 != "././." && $1 != ".|.|." { if(NR==1) {print $2} else {split($2,a,","); for (i in a) {print a[i]/$3} } }' > $GT.AD.txt
